...
function perfex_saas_init()
{
    try {
        if (isset($_SERVER['REQUEST_URI'])) {
            ...
            $options = perfex_saas_get_options(['perfex_saas_enable_client_bridge', 'perfex_saas_enable_cross_domain_bridge', 'perfex_saas_enable_instance_switch', 'perfex_saas_client_bridge_account_menu_position', PERFEX_SAAS_GLOBAL_ACTIVE_MODULES_OPTION_KEY], true);
...
function perfex_saas_simple_query($slug, $sql)
{
    ...
    if (stripos($sql, $master_dbprefix)) {
        ...
        if (stripos($sql, ' ' . $master_dbprefix)) {
            $commonKeywords = array("FROM", "INSERT INTO", "UPDATE", "DELETE FROM", "CREATE TABLE", "ALTER TABLE", "ORDER BY");
...
function perfex_saas_registered_extensions(object $tenant = null, $global_only = true)
{
    ...
    if ($global_only) {
        try {
            $perfex_saas_global_active_modules = $tenant->saas_options[PERFEX_SAAS_GLOBAL_ACTIVE_MODULES_OPTION_KEY];

            if (!empty($perfex_saas_global_active_modules)) {
                $perfex_saas_global_active_modules =  (array)json_decode($perfex_saas_global_active_modules);
            }
        } catch (\Throwable $th) {
