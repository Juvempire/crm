...
class Authentication_model extends App_Model
{
        ...
        if ((!empty($email)) and (!empty($password))) {
            ...
            if ($user) {
                ...
                if (!app_hasher()->CheckPassword($password, $user->password)) {
                    ...
                    log_activity('تلاش برای ورود ناموفق [ایمیل: ' . $email . ', آیا از اعضا هست: ' . ($staff == true ? 'بلی' : 'خیر') . ', آی پی: ' . $this->input->ip_address() . ']');
                ...
                log_activity('کاربر غیر موجود سعی در ورود به سیستم کرد [ایمیل: ' . $email . ', آیا از اعضا هست: ' . ($staff == true ? 'بلی' : 'خیر') . ', آی پی: ' . $this->input->ip_address() . ']');
            ...
            if ($user->active == 0) {
                ...
                log_activity('کاربر غیرفعال سعی کرد وارد شود [ایمیل: ' . $email . ', آیا از اعضا هست: ' . ($staff == true ? 'بلی' : 'خیر') . ', آی پی: ' . $this->input->ip_address() . ']');
    ...
    private function update_login_info($user_id, $staff)
    {
        ...
        log_activity('کاربر با موفقیت وارد سایت شد [شناسه کاربر: ' . $user_id . ', آیا از اعضا هست: ' . ($staff == true ? 'بلی' : 'خیر') . ', آی پی: ' . $this->input->ip_address() . ']');
    }
    ...
    public function forgot_password($email, $staff = false)
    {
        ...
        if ($user) {
            if ($user->active == 0) {
                log_activity('کاربر غیرفعال بازنشانی رمز عبور را امتحان کرده است [ایمیل: ' . $email . ', آیا از اعضا هست: ' . ($staff == true ? 'بلی' : 'خیر') . ', آی پی: ' . $this->input->ip_address() . ']');
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                if ($sent) {
                    log_activity('بازنشانی رمز عبور ایمیل ارسال شد [ایمیل ' . $email . ', آیا از اعضا هست: ' . ($staff == true ? 'بلی' : 'خیر') . ', آی پی: ' . $this->input->ip_address() . ']');
        ...
        log_activity('کاربر غیر موجود بازنشانی رمز عبور را امتحان کرده است [ایمیل: ' . $email . ', آیا از اعضا هست: ' . ($staff == true ? 'بلی' : 'خیر') . ', آی پی: ' . $this->input->ip_address() . ']');
    ...
    public function set_password($staff, $userid, $new_pass_key, $password)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            log_activity('رمز عبور توسط کاربر تنظیم شد [شناسه کاربر: ' . $userid . ', آیا از اعضا هست: ' . ($staff == true ? 'بلی' : 'خیر') . ', آی پی: ' . $this->input->ip_address() . ']');
    ...
    public function reset_password($staff, $userid, $new_pass_key, $password)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            log_activity('کاربر رمز عبور را بازنشانی کرده است [شناسه کاربر: ' . $userid . ', آیا از اعضا هست: ' . ($staff == true ? 'بلی' : 'خیر') . ', آی پی: ' . $this->input->ip_address() . ']');
            $this->db->set('new_pass_key', null);
            $this->db->set('new_pass_key_requested', null);
            $this->db->set('last_password_change', date('Y-m-d H:i:s'));
            $this->db->where($_id, $userid);
